...
namespace WinFormUI
{
    public partial class Dashboard : Form
    {
        ...
        {
            InitializeComponent();
            PupulateCartWithDemoData();
        }

        private void PupulateCartWithDemoData()
        {
        ...
        {
            decimal total = cart.GenerateTotal((subTotal) => textBox2.Text = $"{subTotal:C2}",
                (products, subTotal) => subTotal - (products.Count * 2),
                (message) => { });

            textBox1.Text = $"{total:C2}";
        }
    }
}
